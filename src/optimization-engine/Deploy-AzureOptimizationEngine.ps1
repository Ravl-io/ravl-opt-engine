<#
.SYNOPSIS
This script deploys the RAVLs Optimization Engine.

.DESCRIPTION
This script deploys the RAVLs Optimization Engine to an Azure subscription. The script will guide you through the deployment process, allowing you to choose the subscription,
...
Deploys or upgrades the RAVLs Optimization Engine using the default template from the GitHub repository.
...
.EXAMPLE
.\Deploy-AzureOptimizationEngine.ps1 -TemplateUri "https://raw.githubusercontent.com/Ravl-io/ravl-opt-engine/main/src/optimization-engine/azuredeploy.bicep" -AzureEnvironment "AzureChinaCloud" -ResourceTags @{"Environment"="Production"; "Owner"="John Doe"}

Deploys or upgrades the RAVLs Optimization Engine using a custom template from the GitHub repository, in the AzureChinaCloud environment, with custom resource tags.

.LINK
https://github.com/Ravl-io/ravl-opt-engine/deployment
#>
param (
...
$GitHubOriginalUri = "https://raw.githubusercontent.com/Ravl-io/ravl-opt-engine/main/src/optimization-engine/azuredeploy.bicep"
...
{
    # With a silent deploy, derive resource names from the prefix but keep an explicit
    # resource group name when one was provided by the caller.
...
$deploymentMessage = "Deploying RAVLs Optimization Engine to subscription"
if ($upgrading)
{
    ...
    $deploymentMessage = "Upgrading RAVLs Optimization Engine in subscription"
}
...
if ("Y", "y" -contains $continueInput)
{
    ...
    if (-not($upgrading))
    {
        ...
        Write-Host "Deploying RAVLs Optimization Engine resources..." -ForegroundColor Green
        $deployCostMgmtRole = $true
        try
        {
            $costMgmtRole = Get-AzRoleDefinition -Id "72fafb9e-0641-4f18-9ae6-2d4cd1e3be5d" -ErrorAction SilentlyContinue
            if (-not($costMgmtRole))
            {
                Write-Host "Cost Management Reader role not available in this subscription. Skipping role assignment." -ForegroundColor Yellow
                $deployCostMgmtRole = $false
            }
        }
        catch
        {
        ...
        do
        {
            ...
            try
            {
                $sqlLoc = $(if ($deploymentOptions["SqlLocation"]) { $deploymentOptions["SqlLocation"] } else { $targetLocation })
                $deployment = New-AzDeployment -TemplateFile ".\azuredeploy.bicep" -templateLocation $TemplateUri.Replace("azuredeploy.bicep", "") -Location $targetLocation -rgName $resourceGroupName -Name $deploymentName `
                    -projectLocation $targetlocation -sqlLocation $sqlLoc -logAnalyticsReuse $logAnalyticsReuse -baseTime $baseTime `
                    ...
                    -billingAccountId $(if ($deploymentOptions["BillingAccountId"]) { $deploymentOptions["BillingAccountId"] } else { '' }) `
                    -billingProfileId $(if ($deploymentOptions["BillingProfileId"]) { $deploymentOptions["BillingProfileId"] } else { '' }) `
                    -consumptionScope $(if ($deploymentOptions["ConsumptionScope"]) { $deploymentOptions["ConsumptionScope"] } else { 'BillingAccount' }) `
                    -userPrincipalName $userPrincipalName -userObjectId $userObjectId -sqlAdminPrincipalType $SqlAdminPrincipalType `
                    -resourceTags $ResourceTags -enableDefaultTelemetry $EnableDefaultTelemetry -deployCostMgmtRoleAssignment $deployCostMgmtRole -WarningAction SilentlyContinue
    ...
    else
    {
        ...
        if ($EnableAWS) {
            New-AzStorageContainer -Name "awsexports" -Context $sa.Context -ErrorAction SilentlyContinue | Out-Null
        }
        if ($EnableGCP) {
            New-AzStorageContainer -Name "gcpexports" -Context $sa.Context -ErrorAction SilentlyContinue | Out-Null
        }

        Write-Host "Importing runbooks..." -ForegroundColor Green
        $allRunbooks = $upgradeManifest.baseIngest.runbook + $upgradeManifest.dataCollection.runbook + $upgradeManifest.recommendations.runbook + $upgradeManifest.remediations.runbook + $upgradeManifest.orchestration.runbook
        ...
        $runbooksTemplatePath = "./roe-runbooks-deployment.json"
        ...
        $modulesTemplatePath = "./roe-modules-deployment.json"
        ...
        foreach ($schedule in $allSchedules)
        {
            ...
            $dataExportsToSchedule = ($upgradeManifest.dataCollection + $upgradeManifest.recommendations + $upgradeManifest.orchestration) | Where-Object { $_.exportSchedule -eq $schedule.name }
        ...
        $allVariables = $upgradeManifest.dataCollection.requiredVariables + $upgradeManifest.recommendations.requiredVariables + $upgradeManifest.remediations.requiredVariables + $upgradeManifest.orchestration.requiredVariables
    ...
    Write-Host "Checking Azure Automation variable referring to the initial RAVLs Optimization Engine deployment date..." -ForegroundColor Green
    ...
    do
    {
        ...
        try
        {
            ...
            try {
                ...
                $dbToken = (Get-AzAccessToken -ResourceUrl "https://$azureSqlDomain/").Token
            }
    ...
    if ("Y", "y" -contains $deployWorkbooks)
    {
        ...
        foreach ($workbook in $workbooks)
        {
            ...
            {
                Write-Host "Failed to deploy the workbook. If you are upgrading ROE, please remove first the $workbookFileName workbook from the $laWorkspaceName Log Analytics workspace and then re-deploy." -ForegroundColor Yellow
            }
        }
    }
    #endregion
    ...
    if (!$silentDeploy)
    {
        #region Grant Microsoft Entra ID role to ROE principal
        if ($null -eq $spnId)
        {
            $auto = Get-AzAutomationAccount -Name $automationAccountName -ResourceGroupName $resourceGroupName
            $spnId = $auto.Identity.PrincipalId
        ...
        {
            Write-Host $Error[0] -ForegroundColor Yellow
            Write-Host "Could not grant role. If you want Microsoft Entra-based recommendations, please grant the Global Reader role manually to the $automationAccountName managed identity or, for previous versions of ROE, to the Run As Account principal." -ForegroundColor Red
        }
        #endregion
    }
    else
    ...
    Write-Host "RAVLs Optimization Engine deployment completed! We're almost there..." -ForegroundColor Green
    ...
    if ("Y", "y" -contains $benefitsUsageDependenciesOption)
    {
        ...
        switch ($customerType)
        {
            "EA"
            {
                ...
                Write-Host "Granting the Enterprise Enrollment Reader role to the ROE Managed Identity..." -ForegroundColor Green
            ...
            "MCA"
            {
                ...
                Write-Host "Granting the Billing Profile Reader role to the ROE Managed Identity..." -ForegroundColor Green
