<#
.SYNOPSIS
This script sets up the necessary dependencies for the RAVLs Optimization Engine to collect the data required for the several Azure benefits workbooks.
...
the necessary roles to the RAVLs Optimization Engine managed identity and set up the required variables in the Automation Account.

.PARAMETER AzureEnvironment
The Azure environment to use. Possible values are AzureCloud, AzureChinaCloud, AzureUSGovernment.

.PARAMETER AutomationAccountName
The name of the Automation Account where the RAVLs Optimization Engine is deployed.
...
.LINK
https://github.com/Ravl-io/ravl-opt-engine/commitmentssetup
#>
param(
...
if (-not($scheduledRunbooks)) {
    throw "The $AutomationAccountName Automation Account does not contain any scheduled runbook. It might not be associated to the RAVLs Optimization Engine."
}
...
if (-not($principalId))
{
    throw "The $AutomationAccountName Automation Account does not have a managed identity and probably is not associated to the latest version of RAVLs Optimization Engine. Please, upgrade it before setting up benefits usage dependencies (more details: https://github.com/Ravl-io/ravl-opt-engine/upgrade)."
}
...
if (-not($pricesheetSchedule)) {
    throw "The RAVLs Optimization Engine is in an older version. Please, upgrade it before setting up benefits usage dependencies (more details: https://github.com/Ravl-io/ravl-opt-engine/upgrade)."
}
