...
# $storageAccountSinkContainer ="recommendationstest"
...
do {
    ...
    try {
        ...
        $Cmd.CommandText = "SELECT * FROM [dbo].[$LogAnalyticsIngestControlTable] WHERE CollectedType IN ('ARGVirtualMachine','MonitorMetrics','ARGResourceContainers','AzureConsumption','Pricesheet')"
...
Write-Output "Will run query against tables $vmsTableName, $metricsTableName, $subscriptionsTableName, $pricesheetTableName and $consumptionTableName"
...
Write-Output "Looking for underutilized Virtual Machines, with less than $cpuPercentageThreshold% CPU and $memoryPercentageThreshold% RAM usage..."
...
$baseQuery = @"
    ...
    | where todatetime(Date_s) between (stime..etime) and ResourceId contains 'virtualmachines'
    ...
    $vmsTableName
    | where TimeGenerated > ago(1d)
    | distinct InstanceId_s, VMName_s, ResourceGroupName_s, SubscriptionGuid_g, Cloud_s, TenantGuid_g, VMSize_s, NicCount_s, DataDiskCount_s, CoresCount_s, Tags_s
    ...
    | project InstanceId_s, VMName = VMName_s, ResourceGroup = ResourceGroupName_s, SubscriptionId = SubscriptionGuid_g, Cloud_s, TenantGuid_g, VMSize_s, NicCount_s, DataDiskCount_s, CoresCount_s, PMemoryPercentage, PCPUPercentage, Tags_s, Last30DaysCost, Last30DaysQuantity
    | join kind=leftouter (
...
foreach ($result in $results)
{
    ...
    $targetSku = $null
    $currentSku = $skus | Where-Object { $_.Name -eq $result.VMSize_s }
...
"@
        ...
        $additionalInfoDictionary["currentSku"] = "$($result.VMSize_s)"
        $additionalInfoDictionary["CoresCount"] = [int] $result.CoresCount_s
        ...
        $additionalInfoDictionary["targetSkuSavingsMonthly"] = $targetSkuSavingsMonthly
        $additionalInfoDictionary["targetSkuPrice"] = $targetSkuPrice
        $additionalInfoDictionary["currentSkuPrice"] = $currentSkuPrice
        $additionalInfoDictionary["CurrentCost"]=([double] $result.Last30DaysQuantity * $currentSkuPrice)
        $additionalInfoDictionary["Last30DaysQuantity"]=([double] $result.Last30DaysQuantity)

        $recommendation = New-Object PSObject -Property @{
            Timestamp                   = $timestamp
            Cloud                       = $result.Cloud_s
            Category                    = "Cost"
            ImpactedArea                = "Microsoft.Compute/virtualMachine"
            Impact                      = "High"
            RecommendationType          = "Saving"
            RecommendationSubType       = "UnderusedVMS"
            RecommendationSubTypeId     = "5682b50d-0c31-400f-be6b-8167d9db9453"
            RecommendationDescription   = "Virtual Machine has been underutilized"
            RecommendationAction        = "Resize Virtual Machine to lower SKU or scale it in"
            InstanceId                  = $result.InstanceId_s
            InstanceName                = $result.VMName
...
$jsonExportPath = "vm-underutilized-$fileDate.json"
...
Write-Output "Looking for performance constrained Virtual Machines, with more than $cpuDegradedMaxPercentageThreshold% Max. CPU, $cpuDegradedAvgPercentageThreshold% Avg. CPU and $memoryDegradedPercentageThreshold% RAM usage..."
...
$baseQuery = @"
    ...
    | project InstanceId_s, VMName = VMName_s, ResourceGroup = ResourceGroupName_s, SubscriptionId = SubscriptionGuid_g, Cloud_s, TenantGuid_g, VMSize_s, NicCount_s, DataDiskCount_s, CoresCount_s, PMemoryPercentage, PCPUMaxPercentage, PCPUAvgPercentage, Tags_s
    | join kind=leftouter (
        $subscriptionsTableName
        | where TimeGenerated > ago(1d)
        | where ContainerType_s =~ 'microsoft.resources/subscriptions'
...
"@
    ...
    $recommendation = New-Object PSObject -Property @{
        ...
        RecommendationSubType       = "PerfConstrainedVM"
        RecommendationSubTypeId     = "9be5d68b-b809-497e-b0b2-cfce5d43ab7a"
        RecommendationDescription   = "Virtual Machine performance has been constrained by lack of resources"
        RecommendationAction        = "Resize Virtual Machine to higher SKU or scale it out"
        InstanceId                  = $result.InstanceId_s
...
$fileDate = $datetime.ToString("yyyy-MM-dd")
$jsonExportPath = "vm-perfconstrained-$fileDate.json"
$recommendations | ConvertTo-Json | Out-File $jsonExportPath
