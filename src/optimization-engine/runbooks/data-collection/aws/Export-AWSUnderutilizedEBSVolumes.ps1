...
# Run the AWS underutilized EBS volumes export script
./export-aws-underutilized-ebs-volumes.sh >> logs/aws-underutilized-ebs-volumes_$ts.txt 2>&1
"@

Write-Output "Exporting AWS Underutilized EBS Volumes"
Write-Output "Executing script on VM: $vmName"
Write-Output "Script path: $awsScriptsPath"
Write-Output "Log file: logs/aws-underutilized-ebs-volumes_$ts.txt"

# Invoke-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -CommandId 'RunShellScript' -ScriptString $script -ErrorAction Stop
Set-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -Location $vmLocation -RunCommandName "ExportAWSUnderutilizedEBS" –SourceScript $script

Write-Output "Completed Exporting AWS Underutilized EBS Volumes"
