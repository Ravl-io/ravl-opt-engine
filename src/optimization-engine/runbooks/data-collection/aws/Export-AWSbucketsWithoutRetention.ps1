...
# Run the AWS buckets without retention export script
./export-aws-buckets-no-retention.sh >> logs/aws-buckets-no-retention_$ts.txt 2>&1
"@

Write-Output "Exporting AWS Buckets without Retention Policies"
Write-Output "Executing script on VM: $vmName"
Write-Output "Script path: $awsScriptsPath"
Write-Output "Log file: logs/aws-buckets-no-retention_$ts.txt"

# Invoke-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -CommandId 'RunShellScript' -ScriptString $script -ErrorAction Stop
Set-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -Location $vmLocation -RunCommandName "ExportAWSBucketWithoutRetention" –SourceScript $script

Write-Output "Completed Exporting AWS Buckets without Retention Policies"
