...
# Run the AWS idle ECS services export script
./export-aws-idle-ecs-services.sh >> logs/aws-idle-ecs-services_$ts.txt 2>&1
"@

Write-Output "Exporting AWS Idle ECS Services"
Write-Output "Executing script on VM: $vmName"
Write-Output "Script path: $awsScriptsPath"
Write-Output "Log file: logs/aws-idle-ecs-services_$ts.txt"

# Invoke-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -CommandId 'RunShellScript' -ScriptString $script -ErrorAction Stop
Set-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -Location $vmLocation -RunCommandName "ExportAWSIdleECSService" -SourceScript $script

Write-Output "Completed Exporting AWS Idle ECS Services"
