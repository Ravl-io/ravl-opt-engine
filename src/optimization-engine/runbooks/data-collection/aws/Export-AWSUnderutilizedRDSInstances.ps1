...
# Run the AWS underutilized RDS instances export script
./export-aws-underutilized-rds-instances.sh >> logs/aws-underutilized-rds-instances_$ts.txt 2>&1
"@

Write-Output "Exporting AWS Underutilized RDS Instances"
Write-Output "Executing script on VM: $vmName"
Write-Output "Script path: $awsScriptsPath"
Write-Output "Log file: logs/aws-underutilized-rds-instances_$ts.txt"

# Invoke-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -CommandId 'RunShellScript' -ScriptString $script -ErrorAction Stop
Set-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -Location $vmLocation -RunCommandName "ExportAWSUnderutilizedRDS" –SourceScript $script

Write-Output "Completed Exporting AWS Underutilized RDS Instances"
