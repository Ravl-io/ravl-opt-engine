...
# Run the AWS Underutilized Load Balancers export script
./export-aws-underutilized-load-balancers.sh >> logs/aws-underutilized-load-balancers_$ts.txt 2>&1
"@

Write-Output "Exporting AWS Underutilized Load Balancers"
Write-Output "Executing script on VM: $vmName"
Write-Output "Script path: $awsScriptsPath"
Write-Output "Log file: logs/aws-underutilized-load-balancers_$ts.txt"

Set-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -Location $vmLocation -RunCommandName "ExportAWSUnderutilizedLoadBalancers" –SourceScript $script

Write-Output "Completed Exporting AWS Underutilized Load Balancers"
