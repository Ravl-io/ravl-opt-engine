...
# Run the AWS idle Lambda Functions export script
./export-aws-idle-lambda-functions.sh >> logs/aws-idle-lambda-functions_$ts.txt 2>&1
"@

Write-Output "Exporting AWS Idle Lambda Functions"
Write-Output "Executing script on VM: $vmName"
Write-Output "Script path: $awsScriptsPath"
Write-Output "Log file: logs/aws-idle-lambda-functions_$ts.txt"

Set-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -Location $vmLocation -RunCommandName "ExportAWSIdleLambdaFunctions" –SourceScript $script

Write-Output "Completed Exporting AWS Idle Lambda Functions"
