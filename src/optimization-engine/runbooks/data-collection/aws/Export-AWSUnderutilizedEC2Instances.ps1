...
# Run the AWS underutilized EC2 instances export script
./export-aws-underutilized-ec2-instances.sh >> logs/aws-underutilized-ec2-instances_$ts.txt 2>&1
"@

Write-Output "Exporting AWS Underutilized EC2 Instances"
Write-Output "Executing script on VM: $vmName"
Write-Output "Script path: $awsScriptsPath"
Write-Output "Log file: logs/aws-underutilized-ec2-instances_$ts.txt"

# Invoke-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -CommandId 'RunShellScript' -ScriptString $script -ErrorAction Stop
Set-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -Location $vmLocation -RunCommandName "ExportAWSUnderutilizedEC2" –SourceScript $script

Write-Output "Completed Exporting AWS Underutilized EC2 Instances"
