...
# Run the AWS unattached EIPs export script
./export-aws-unattached-eips.sh >> logs/aws-unattached-eips_$ts.txt 2>&1
"@

Write-Output "Exporting AWS Unattached EIPs"
Write-Output "Executing script on VM: $vmName"
Write-Output "Script path: $awsScriptsPath"
Write-Output "Log file: logs/aws-unattached-eips_$ts.txt"

# Invoke-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -CommandId 'RunShellScript' -ScriptString $script -ErrorAction Stop
Set-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -Location $vmLocation -RunCommandName "ExportAWSUnattachedIPs" –SourceScript $script

Write-Output "Completed Exporting AWS Unattached EIPs"
