...
./export-idle-gcp-disks.sh >> logs/gcp-idle-disks_$ts.txt 2>&1
"@

Write-Output "Exporting GCP Idle Disks"
...
Set-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -Location $vmLocation -RunCommandName "ExportGCPIdleDisks" –SourceScript $script

Write-Output "Completed Exporting GCP Idle Disks"
