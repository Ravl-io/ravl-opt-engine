...
./export-underutilized-gcp-vms-offhours.sh >> logs/gcp-underutilized-vms-off-hours_$ts.txt 2>&1
"@

Write-Output "Exporting GCP Underutilized VMs in off-hours"
...
Set-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -Location $vmLocation -RunCommandName "ExportGCPUnderutilizedVMsOffHours" –SourceScript $script

Write-Output "Completed Exporting GCP Underutilized VMs in off-hours"
