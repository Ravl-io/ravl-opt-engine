...
./export-unmanaged-gcp-disks.sh >> logs/gcp-unmanaged-disks_$ts.txt 2>&1
"@

Write-Output "Exporting GCP Unmanaged Disks"
...
Set-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -Location $vmLocation -RunCommandName "ExportGCPUnmanagedDisks" –SourceScript $script

Write-Output "Completed Exporting GCP Unmanaged Disks"
