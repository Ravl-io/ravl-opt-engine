...
./export-underutilized-gcp-cloudsql.sh >> logs/export-underutilized-gcp-cloudsql_$ts.txt 2>&1
"@

Write-Output "Exporting GCP underutilized Cloud SQL instances"
...
Set-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -Location $vmLocation -RunCommandName "ExportGCPUnderutilizedSQL" –SourceScript $script

Write-Output "Completed Exporting GCP underutilized Cloud SQL instances"
