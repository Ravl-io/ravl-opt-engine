...
./export-idle-services-gcp.sh >> logs/export-idle-services-gcp_$ts.txt 2>&1
"@

Write-Output "Exporting GCP idle services"
...
Set-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -Location $vmLocation -RunCommandName "ExportGCPIdleServices" –SourceScript $script

Write-Output "Completed Exporting GCP idle services"
