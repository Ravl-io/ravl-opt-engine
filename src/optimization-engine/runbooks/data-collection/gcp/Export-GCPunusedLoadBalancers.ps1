...
./export-unused-gcp-load-balancers.sh >> logs/export-unused-gcp-load-balancers_$ts.txt 2>&1
"@

Write-Output "Exporting GCP unused load balancers"
...
Set-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -Location $vmLocation -RunCommandName "ExportGCPUnusedLoadBalancers" –SourceScript $script -TimeoutInSecond 12800

Write-Output "Completed Exporting GCP unused load balancers"
