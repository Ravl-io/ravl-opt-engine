...
./export-bucketswithoutretention-gcp.sh >> logs/export-bucketswithoutretention-gcp_$ts.txt 2>&1
"@

Write-Output "Exporting GCP buckets without retention"
...
Set-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -Location $vmLocation -RunCommandName "ExportGCPBucketsWithoutRetention" –SourceScript $script

Write-Output "Completed Exporting GCP buckets without retention"
