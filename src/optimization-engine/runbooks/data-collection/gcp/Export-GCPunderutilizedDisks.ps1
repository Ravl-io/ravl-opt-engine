...
./export-underutilized-gcp-disks.sh >> logs/export-underutilized-gcp-disks_$ts.txt 2>&1
"@

Write-Output "Exporting GCP underutilized disks"
...
Set-AzVMRunCommand -ResourceGroupName $rg -VMName $vmName -Location $vmLocation -RunCommandName "ExportGCPUnderutilizedDisks" –SourceScript $script -TimeoutInSecond 14800

Write-Output "Completed Exporting GCP underutilized disks"
