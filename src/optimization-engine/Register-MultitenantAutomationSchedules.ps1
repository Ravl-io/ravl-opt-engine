<#
.SYNOPSIS
This script adds job schedules for all RAVLs Optimization Engine data collection runbooks for a given tenant/cloud.

.DESCRIPTION
This script adds job schedules for all RAVLs Optimization Engine data collection runbooks for a given tenant/cloud.
It is useful when you want to collect data from multiple tenants/clouds in a single ROE deployment.
...
You also need to create an Automation Credential in the ROE Automation Account with the service principal credentials (client ID and secret).
...
The name of the Automation Credential in the ROE Automation Account containing the service principal credentials (client ID and secret).
...
.EXAMPLE
Register-MultitenantAutomationSchedules.ps1 -AutomationAccountName "ROE" -ResourceGroupName "ROE-RG" -TargetSchedulesSuffix "-Tenant1" -TargetTenantId "00000000-0000-0000-0000-000000000000" -TargetTenantCredentialName "Tenant1" -ExcludedRunbooks @("Export-ReservationsPriceToBlobStorage")

.LINK
https://github.com/Ravl-io/ravl-opt-engine/customize
#>
param(
...
{
    throw "The $AutomationAccountName Automation Account does not contain any scheduled data collection runbook. It might not be associated to the RAVLs Optimization Engine."
}
